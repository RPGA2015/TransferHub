...
function Results({ corridor, providers, visibleResultCount, sortBy, payoutFilter, onSortChange, onFilterChange, onSelect, selectedProvider, buttonRefs }: ResultsProps) {
  ...
  const summary = visibleResultCount > 0
    ? `Showing ${visibleResultCount} illustrative ${visibleResultCount === 1 ? "option" : "options"}. Recipient amounts range from ${formatRecipientAmount(Math.min(...recipientAmounts), corridor.receivingCurrency)} to ${formatRecipientAmount(Math.max(...recipientAmounts), corridor.receivingCurrency)}.`
